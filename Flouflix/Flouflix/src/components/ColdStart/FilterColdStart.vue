<template>
    <el-form class="filter rounded-border">
        <div class="filter-item">
            <p>Type: </p>
            <el-form-item>
                <el-select filterable clearable v-model="mlStore.coldStartFilter.type">
                    <el-option v-for="item in typeOptions" :key="item.key" :label="item.type" :value="item.type" />
                </el-select>
            </el-form-item>
        </div>
        <div class="filter-item">
            <p>Mécanique: </p>
            <el-form-item>
                <el-select filterable clearable v-model="mlStore.coldStartFilter.mechanic">
                    <el-option v-for="item in this.mechOptions" :key="item.key" :label="item.mechanic"
                        :value="item.mechanic" />
                </el-select>
            </el-form-item>
        </div>
        <div class="filter-item">
            ...
            <el-form-item class="slider">
                <el-slider :step="20" :max="180" :format-tooltip="durationTooltip"
                    v-model="mlStore.coldStartFilter.duration" />
            </el-form-item>
        </div>
        <div class="filter-item">
            <p> Age minimum: </p>
            <el-form-item class="slider">
                <el-slider :min="3" :max="16" :format-tooltip="ageTooltip" v-model="mlStore.coldStartFilter.age" />
            </el-form-item>
        </div>
        <ColdButton class = "filter-item filterButton"/>    
    </el-form>
</template>
<script>
import { ref } from 'vue';
...
import ColdButton from '@/components/ColdStart/ColdButton.vue';

export default {
    name: "FilterColdStart",
    setup() {
        const form = ref({
            type: "",
            mechanic: "",
